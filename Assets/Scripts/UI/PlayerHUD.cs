...
{
    [Header("References")]
    public PlayerCharacter player;  // CHANGED: PlayerController → PlayerCharacter
    ...
    public WaveformUI waveformAnimation;
    ...
    private void Start()
    {
        // CHANGED: Find PlayerCharacter instead of PlayerController
        if (player == null)
        {
            // Try to get it from the Player singleton first
            if (Player.Instance != null)
            {
                player = Player.Instance.GetComponent<Player>().GetComponentInChildren<PlayerCharacter>();
            }
        ...
        {
            Debug.LogError("PlayerHUD: Could not find PlayerCharacter!");
        }
    }
